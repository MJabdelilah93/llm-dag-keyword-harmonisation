"""Tkinter view layer for the M7 annotation GUI. Thin -- all state lives
in session.AnnotationSession. Never exposes stratum/scores/frequencies/
routes/model predictions/other-annotator labels/full source records."""
...
from .session import ALLOWED_LABELS, AnnotationSession
...
class App:
    def __init__(self, session: AnnotationSession, context: CombinedContextLookup, annotator_label: str):
        ...
        self.root = tk.Tk()
        self.root.title(f"M7 Human Annotation - {annotator_label}")
        self.root.geometry("1100x700")
        self.root.minsize(1024, 700)
        self.root.configure(bg=BG)
        self.root.protocol("WM_DELETE_WINDOW", self._on_close)

        self._justification_var = tk.StringVar()
    ...
    def _build_home_screen(self):
        ...
        tk.Label(frame, text="M7 Human Annotation", font=("Segoe UI", 26, "bold"), bg=BG, fg=FG).pack(pady=(60, 10))
    ...
    def _build_annotation_screen(self):
        ...
        tk.Label(top, text="M7 Human Annotation", font=("Segoe UI", 12, "bold"), bg=BG, fg=FG).pack(side="left")
        ...
        idx_label = tk.Label(top, text=f"Pair {self.session.current_index + 1} of {total}", font=("Segoe UI", 12), bg=BG, fg=FG)
        ...
        tk.Label(center, text="KEYWORD A", font=("Segoe UI", 13, "bold"), bg=BG, fg=MUTED).pack(pady=(10, 0))
        a_text = "\n".join(textwrap.wrap(pair.string_a, width=60)) or pair.string_a
        ...
        tk.Label(center, text="KEYWORD B", font=("Segoe UI", 13, "bold"), bg=BG, fg=MUTED).pack(pady=(10, 0))
        b_text = "\n".join(textwrap.wrap(pair.string_b, width=60)) or pair.string_b
        tk.Label(center, text=b_text, font=("Segoe UI", 26), bg=BG, fg=FG, wraplength=900, justify="center").pack(pady=(0, 10))

        tk.Label(center, text=f"pair_id: {pair.pair_id}", font=("Segoe UI", 8), bg=BG, fg="#BBBBBB").pack(side="bottom", anchor="w")
        ...
        tk.Button(aux_frame, text="ADD NOTE / JUSTIFICATION (J)", bg=BTN_NEUTRAL, command=self._open_justification).pack(side="left", padx=6)
        ...
        tk.Label(
            ...
            text="Shortcuts: 1/M=Match  2/N=Non-match  3/U=Uncertain  C=Context  J=Note  ←/B=Back  →=Next  S=Skip",
    ...
    def _open_context(self):
        ...
        for label, string in (("Keyword A", pair.string_a), ("Keyword B", pair.string_b)):
        ...
        # refresh the underlying screen's context_used display once closed
        popup.wait_window()
        self._build_annotation_screen()
    ...
    def _open_justification(self):
        pair = self.session.current_pair()
        popup = tk.Toplevel(self.root)
        popup.title("Justification")
        popup.configure(bg=BG)
        popup.geometry("500x260")
        popup.bind("<Escape>", lambda e: popup.destroy())
        popup.grab_set()

        tk.Label(popup, text="Justification (optional)", font=("Segoe UI", 12, "bold"), bg=BG, fg=FG).pack(anchor="w", padx=16, pady=(12, 4))
    ...
    def _build_complete_screen(self):
        ...
        frame = tk.Frame(self.root, bg=BG)
        frame.pack(expand=True, fill="both")
        tk.Label(frame, text="ANNOTATION COMPLETE", font=("Segoe UI", 26, "bold"), bg=BG, fg="#1B7A2E").pack(pady=(120, 20))
        tk.Label(frame, text=f"All {len(self.session.pairs)} pairs have a label.", font=("Segoe UI", 13), bg=BG, fg=FG).pack()
        if completed_path:
    ...
    def _build_review_screen(self):
        ...
        tk.Label(top, text="Review Completed / In-Progress Pairs", font=("Segoe UI", 14, "bold"), bg=BG, fg=FG).pack(side="left")
        ...
        columns = ("pair_id", "domain", "string_a", "string_b", "label", "context_used")
        tree = ttk.Treeview(self.root, columns=columns, show="headings", height=20)
        for c in columns:
            tree.heading(c, text=c)
            tree.column(c, width=140 if c in ("string_a", "string_b") else 100)
        ...
        def refresh(*_):
            ...
            for p in self.session.pairs:
                ...
                if f == "unresolved" and p.label:
                    continue
                tree.insert("", "end", iid=p.pair_id, values=(p.pair_id, DOMAIN_LABELS.get(p.domain, p.domain), p.string_a, p.string_b, p.label, p.context_used))
    ...
    def _on_close(self):
        self.session.backup_now() if any(p.label for p in self.session.pairs) else None
        self.root.destroy()
