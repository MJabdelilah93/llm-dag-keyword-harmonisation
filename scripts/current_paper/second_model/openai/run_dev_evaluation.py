...
========================
PHASE 1B / TASK 5 -- second-model (OpenAI gpt-5.4-nano-2026-03-17)
evaluation on the frozen DEVELOPMENT SET ONLY (N=351). Selects the
confidence threshold that maximises F1 subject to coverage >= 0.70 on
dev, using the identical pre-specified rule and grid already used for the
primary model and for the (unexecuted) Gemini condition.

One real API call per pair; the confidence threshold sweep is then a pure
post-hoc replay of apply_guard() over the already-collected responses --
no extra API calls per threshold candidate.

`--mode real` is refused unless explicitly unlocked (see
--i-have-authorisation). `--mode dry-run` uses the synthetic MockClient
...
from llm_client import OpenAIClient, MockClient, MODEL_ID
...
def main():
    ...
    out_dir = repo_root / "results" / "current_paper" / "second_model" / "openai" / f"dev_run_{args.run_id}"
    if out_dir.exists():
    ...
    manifest = {
        ...
        "total_reasoning_tokens": sum(r.get("thinking_tokens") or 0 for r in records),
