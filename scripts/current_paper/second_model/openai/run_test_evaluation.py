...
=========================
PHASE 1B / TASK 7 -- second-model (OpenAI gpt-5.4-nano-2026-03-17)
evaluation on the frozen HELD-OUT TEST SET, exactly once, using the
threshold already frozen by run_dev_evaluation.py. Refuses to run without
an explicit --frozen-threshold value AND a --dev-manifest reference for
provenance; refuses to run twice into the same --run-id (immutability);
refuses --mode real without --i-have-authorisation.
"""
import argparse
import csv
import hashlib
...
def main():
    ...
    out_dir = repo_root / "results" / "current_paper" / "second_model" / "openai" / f"test_run_{args.run_id}"
    if out_dir.exists():
